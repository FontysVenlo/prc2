...
.icon:linux[fw] install openjdk via apt-get
[source,sh]
----
...
The example shows the archive name as of 10 February 2023,
...
cd /usr/lib/jvm
sudo tar xf ~/Downloads/OpenJDK17U-jdk_x64_linux_hotspot_17.0.6_10.tar.gz
----
...
To install it, do (example uses version 3.9.0  as of 2023-02-06)
...
cd /usr/share
sudo rm -fr maven
sudo tar xf ~/Downloads/apache-maven-3.9.0-bin.tar.gz
sudo ln -sf apache-maven-3.9.0/ maven
cd /usr/bin
...
The official Java IDE at Fontys ICT Venlo is https://netbeans.apache.org/[Apache NetBeans IDE]. The long time stable version for the moment is 16.0.
...
----
cd /usr/local
sudo unzip ~/Downloads/netbeans-16-bin.zip
...
----
#!/usr/bin/env bash
/usr/local/netbeans-16/bin/netbeans "$@" & #<1>
----
...
cd /usr/local/bin #<1>
touch netbeans #<2>
echo \"/Applications/NetBeans/Apache\ NetBeans\ 16.app/Contents/Resources/NetBeans/netbeans/bin/netbeans\" --jdkhome 
