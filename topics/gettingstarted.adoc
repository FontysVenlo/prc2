...
the Java distribution from  https://adoptium.net/temurin/releases/?version=25[AdoptOpenJDK ^].
For PRC2 we will use [blue]*Java 25*.
...
https://adoptium.net/temurin/releases/?version=25[AdoptOpenJDK ^].
...
[source,sh]
----
export JAVA_HOME=/usr/lib/jvm/java-25-openjdk-amd64 #<1>
export JAVA_HOME=/usr/lib/jvm/temurin-25-jdk #<2>
----
...
alias j25="export JAVA_HOME=`/usr/libexec/java_home -v 25.0.1`; java -version"
...
.icon:apple[fw] Open `myProject` in Netbeans using Java version 25
[source,sh]
----
j25
netbeans myProject
