...
image:DatabaseInLayeredArchitecture.png[width=50%, title="Database in layered architecture"] Database in layered architecture.
...
.[black]#db.properties# file
[source,text]
----
include::code/simplejdbc/db.properties[]
----
...
<3> The method calls `doQuery`, which uses the DataSource field to obtain its own connection in a try-with-resources block +
...
<1> Values correspond to the columns in the insert statement: `student_id`, `lastname`, `firstname`, `dob`, `gender`.
