...
// Manually checking
boolean isPublic = (modifiers & 1) != 0;// Modifier.PUBLIC == 1
boolean isFinal = (modifiers & 16) != 0;// Modifier.FINAL == 16
boolean isPublicFinal = (modifiers & 17) == 17; // Modifier.PUBLIC | Modifier.FINAL == 17
----
...
Beyond inspecting a single class, reflection lets you walk the full
class hierarchy at runtime.

.Class hierarchy traversal
[source,java]
----
// Get the direct superclass (returns null for Object and interfaces)
Class<?> superClass = ArrayList.class.getSuperclass();
// -> class java.util.AbstractList

// Get the interfaces directly implemented (or extended, for interfaces)
Class<?>[] interfaces = ArrayList.class.getInterfaces();
// -> [RandomAccess, Cloneable, Serializable, List]

// Check whether a Class object represents an interface
boolean isInterface = Runnable.class.isInterface();  // true
boolean isClass     = ArrayList.class.isInterface(); // false
----

When iterating over fields you can also ask a `Field` for its declared type
and the class it originally came from.

.Field type and origin
[source,java]
----
...
// The class that declared the field (useful when fields come from supertypes)
...
Other annotations you know from other libraries such as:
...
The previous example works well when you know exactly which annotation you are
looking for. Sometimes you want to process *all* annotations on a field without
knowing their types upfront. For this, use `getAnnotations()` and dispatch on
`annotationType()`.

.Processing all annotations on a field
[source,java]
----
for (Annotation annotation : field.getAnnotations()) {    // <1>
    if (annotation.annotationType() == Hidden.class) {    // <2>
        Hidden hidden = (Hidden) annotation;              // <3>
        System.out.println(hidden.value());
    }
}
----
<1> `getAnnotations()` returns every annotation present on the field.
<2> `annotationType()` returns the annotation's class, used to dispatch.
<3> Cast to the concrete annotation type to access its parameters.
