...
*In all cases*, the System Under Test, the class we are testing *is always used as is*. So we have no stunt doubles for our protagonist.
...
<6> And the expected data is passed to the printer, so a real printer would
...
The trick her is to use one of the constructors to given PrintStream something to 'print on', which you can inspect afterwards.
...
method such as the well known `public static void main(String[])` that can't be used with Dependency Inversion as we do here.
****
